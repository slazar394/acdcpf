...
def from_matacdc(ppc: dict, pdc: dict, name: str = "",
    ...
    **DC-DC converters** -- DC branches with ``N != 1`` are treated as
    ...
    for row in pdc["convdc"]:
        ...
        vcmax = float(row[12])   # VCMAX: max converter voltage magnitude (pu)
        vcmin = float(row[13])   # VCMIN: min converter voltage magnitude (pu)
        imax = float(row[14])    # ICMAX: max converter current (pu)
        status = int(row[15])
        loss_a = float(row[16])
        loss_b = float(row[17])
        ...
        create_vsc(
            ...
            i_max_pu=imax if imax > 0 else None,
            vc_max_pu=vcmax if vcmax > 0 else None,
            vc_min_pu=vcmin if vcmin > 0 else None,
